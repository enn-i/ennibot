...
void reply_noping(struct discord *client, const struct discord_message *event,
                  char *s) {
...
// replies to person who sent the message's reply and deletes the person's
// message
//  (lets person send message as EnniBot)
void sm_reply_reply_delete(struct discord *client,
                           const struct discord_message *event, char *s) {
  struct discord_create_message params = { 
		 .content = s,
  ...
  discord_delete_message(client, event->channel_id, event->id,
                         &(struct discord_delete_message){.reason = "echo!"},
                         NULL);
  discord_create_message(client, event->channel_id, &params, NULL);
}
...
void d_timestamp(char *c, time_t t, char *mode) {
  char m;
  if (mode == NULL || !(mode = strpbrk(mode, "tTdDfFR")))
    m = 'f';
  else
    m = mode[0];
  if (c != NULL)
    sprintf(c, "<t:%jd:%c>", (intmax_t)t, m);
}
...
void p_time(char *c, time_t *t) {
  if (t == NULL)
    *t = time(NULL);
  if (c != NULL)
    sprintf(c, "%jd", (intmax_t)(*t));
}

void bw_detect(char *message, struct discord *client,
               const struct discord_message *event) {
  char bw = 0;
  for (char *mp = message; *mp; ++mp)
    *mp = tolower(*mp);

  if (strstr(message, "french"))
    bw = 1;
...
char dgm_setup(struct discord *client, const struct discord_message *msg,
               struct discord_guild_members *members, u64snowflake start) {
  struct discord_ret_guild_members rgm = {.sync = members};
  struct discord_list_guild_members parameters = {.limit = 1000,
                                                  .after = start};
  CCORDcode code;
...
struct discord_guild_member *find_mem(struct discord *client,
                                      const struct discord_message *msg,
                                      struct discord_guild_members *members,
                                      char *username, u64snowflake *uuid) {
